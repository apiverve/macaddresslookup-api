/*
 * MAC Address Lookup API - Basic Usage Example
 *
 * This example demonstrates the basic usage of the MAC Address Lookup API.
 * API Documentation: https://docs.apiverve.com/ref/macaddresslookup
 */
...
{
    class MACAddressLookupExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/macaddresslookup";

        /// <summary>
        /// Make a GET request to the MAC Address Lookup API
        /// </summary>
        static async Task<JsonDocument> CallMACAddressLookupAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling MAC Address Lookup API...\n");

            var result = await CallMACAddressLookupAPI();
